...
export default function Cart({ className = '', cart, handleBuy, handleRemove }) {
    ...
    const priceTotal = useMemo(() => ( cart.reduce((total, product) => total + (product.price * product.quantity), 0)), [cart])
    ...
    return (
        <div className={`${className} flow_0_25`}>
            ...
            <div className="container CartPriceContainer">
                <h2 className="CartPriceLabel">TOTAL BEFORE TAXES{"\u00A0".repeat(3)}</h2>
